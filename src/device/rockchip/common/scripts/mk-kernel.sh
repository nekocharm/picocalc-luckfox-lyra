...
do_build()
{
	...
	case "$1" in
		kernel-config | kconfig)
			...
			if [ -L "$KERNEL_CONFIG_DIR/$RK_KERNEL_CFG" ]; then
				KERNEL_REAL_DEFCONFIG=$(readlink -f "$KERNEL_CONFIG_DIR/$RK_KERNEL_CFG")
				run_command mv kernel/defconfig \
					"$KERNEL_REAL_DEFCONFIG"
				run_command	ln -sfr "$KERNEL_REAL_DEFCONFIG" \
